...
class Rollout2DUNetCallback(Callback):
    ...
    def on_validation_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule) -> None:
        ...
        with torch.no_grad():
            ...
            relative_errors = [float(relative_error / relative_error_normalizer) for relative_error in relative_errors]

            absolute_error_normalizer = np.mean(np.array(absolute_error_normalizer_list))
            absolute_errors = [float(absolute_error / absolute_error_normalizer) for absolute_error in absolute_errors]
