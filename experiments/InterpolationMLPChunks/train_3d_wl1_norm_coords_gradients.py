...
from leap3d.models.lightning import InterpolationMLPChunks, InterpolationMLPChunks3D
from leap3d.scanning.scan_parameters import ScanParameters
from leap3d.train import train_model
from leap3d.transforms import normalize_extra_param, normalize_temperature_3d, normalize_temporal_grad, scanning_angle_cos_transform, normalize_positional_grad
...
def train():
    ...
    train_transforms = {
        'input': transforms.Compose([
            torch.tensor,
            transforms.Lambda(lambda x: normalize_temperature_3d(x, melting_point=TEMPERATURE_MAX, base_temperature=BASE_TEMPERATURE, inplace=True)),
    ...
    inverse_transforms = {
        'input': transforms.Compose([
            torch.tensor,
            transforms.Lambda(lambda x: normalize_temperature_3d(x, melting_point=TEMPERATURE_MAX, base_temperature=BASE_TEMPERATURE, inverse=True, inplace=True))
        ]),
        'target': transforms.Compose([
            torch.tensor,
            transforms.Lambda(lambda x: normalize_extra_param(x, index=3, min_value=BASE_TEMPERATURE, max_value=TEMPERATURE_MAX, inplace=True, inverse=True)),
        ])
    }
    ...
    model = InterpolationMLPChunks3D(**hparams)
