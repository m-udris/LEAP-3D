...
def generate_case_dataset(path, case_id, force_prepare=False):
    ...
    datamodule = MLPInterpolationChunkDataModule(
        ...
        input_shape=[24,24],
        target_shape=[6],
        input_channels=[OffsetLowResRoughTemperatureAroundLaser(is_3d=False, box_size=24, return_coordinates=True)],
        extra_input_channels=[ScanningAngle, LaserPower, LaserRadius],
        target_channels=[MeltPoolPointChunk(is_3d=False, chunk_size=24*24, input_shape=[24,24], include_gradients=True)],
        transforms={},
        inverse_transforms={},
...
if __name__ == '__main__':
    train_cases = list(range(40)) + list(range(100, 140))
    test_cases = list(range(40, 44)) + list(range(140, 144))

    generate_cases(DATASET_DIR / 'mlp_interpolation_chunks_offset_coordinates_gradients', train_cases + test_cases, force_prepare=True)
    aggregate_datasets(DATASET_DIR / 'mlp_interpolation_chunks_offset_coordinates_gradients', train_cases, is_test=False)
    aggregate_datasets(DATASET_DIR / 'mlp_interpolation_chunks_offset_coordinates_gradients', test_cases, is_test=True)
