...
def generate_case_dataset(path, case_id, force_prepare=False, is_eval=False):
    ...
    datamodule = UNetForecastingDataModule(
        ...
        input_shape=[64, 64],
        target_shape=[64, 64],
        input_channels=[LaserPosition(is_3d=False), RoughTemperature(is_3d=False)],
        extra_input_channels=[ScanningAngle, LaserPower, LaserRadius],
        target_channels=[RoughTemperatureDifference(is_3d=False)],
        transforms={},
        inverse_transforms={},
...
if __name__ == '__main__':
    ...
    generate_cases(DATASET_DIR / 'unet_forecasting', train_cases + test_cases + eval_cases, force_prepare=False, eval_cases=eval_cases)
    aggregate_datasets(DATASET_DIR / 'unet_forecasting', train_cases, is_test=False)
    aggregate_datasets(DATASET_DIR / 'unet_forecasting', test_cases, is_test=True)
