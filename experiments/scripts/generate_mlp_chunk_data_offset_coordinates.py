...
def generate_case_dataset(path, case_id, force_prepare=False):
    ...
    datamodule = MLPInterpolationChunkDataModule(
        ...
        input_shape=[24, 24],
        target_shape=[3],
        input_channels=[OffsetLowResRoughTemperatureAroundLaser(is_3d=False, box_size=24, offset_ratio=2/3, return_coordinates=True)],
...
if __name__ == '__main__':
    train_cases = list(range(90)) + list(range(100, 190))
    test_cases = list(range(90, 100)) + list(range(190, 200))

    generate_cases(DATASET_DIR / 'mlp_interpolation_chunks_offset_coordinates', train_cases + test_cases, force_prepare=False)
    aggregate_datasets(DATASET_DIR / 'mlp_interpolation_chunks_offset_coordinates', train_cases, is_test=False)
    aggregate_datasets(DATASET_DIR / 'mlp_interpolation_chunks_offset_coordinates', test_cases, is_test=True)
