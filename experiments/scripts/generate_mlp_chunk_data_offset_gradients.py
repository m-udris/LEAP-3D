...
def generate_case_dataset(path, case_id, force_prepare=False):
    ...
    datamodule = MLPInterpolationChunkDataModule(
        ...
        input_shape=[24,24],
        target_shape=[6],
        input_channels=[OffsetLowResRoughTemperatureAroundLaser(is_3d=False, box_size=24, return_coordinates=False)],
...
if __name__ == '__main__':
    ...
    generate_cases(DATASET_DIR / 'mlp_interpolation_chunks_offset_gradients', train_cases + test_cases, force_prepare=True)
    aggregate_datasets(DATASET_DIR / 'mlp_interpolation_chunks_offset_gradients', train_cases, is_test=False)
    aggregate_datasets(DATASET_DIR / 'mlp_interpolation_chunks_offset_gradients', test_cases, is_test=True)
