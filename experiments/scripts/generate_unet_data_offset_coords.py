...
def generate_case_dataset(path, case_id, force_prepare=False):
    ...
    datamodule = UNetInterpolationDataModule(
        ...
        input_channels=[OffsetRoughTemperatureAroundLaser(return_coordinates=True)],
...
if __name__ == '__main__':
    ...
    generate_cases(DATASET_DIR / 'unet_interpolation_offset_coordinates', train_cases + test_cases, force_prepare=True)
    aggregate_datasets(DATASET_DIR / 'unet_interpolation_offset_coordinates', train_cases, is_test=False)
    aggregate_datasets(DATASET_DIR / 'unet_interpolation_offset_coordinates', test_cases, is_test=True)
