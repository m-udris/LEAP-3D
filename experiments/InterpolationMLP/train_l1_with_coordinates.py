...
def train():
    dataset_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else DATASET_DIR / 'mlp_interpolation_no_distances'
    ...
    hparams = {
        ...
        'tags': ['MLP', '2D', 'interpolation', 'l1_loss', 'with_coordinates'],
        'force_prepare': False,
        'is_3d': False,
    ...
    wandb_config = {
        ...
        'name': f'interpolation_mlp_2d_l1_loss_b{hparams["batch_size"]}_with_coordinates',
    ...
    callbacks = [
        get_checkpoint_only_last_epoch_callback(checkpoint_filename, monitor='val_loss', mode='min', filename='mlp_l1_coords')
    ]
