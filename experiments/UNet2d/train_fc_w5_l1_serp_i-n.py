...
target_to_train = transforms.Compose([
        transforms.Lambda(lambda x: get_target_to_train_transform(TEMPERATURE_MIN, TEMPERATURE_MAX, 0, 10)(x, inplace=True))
    ])


def train_unet2d_param_window_normalized(experiment_name='fc_w5ws1_input_norm', window_size=5, window_step_size=5, max_epochs=50, *args, **kwargs):
    train(
